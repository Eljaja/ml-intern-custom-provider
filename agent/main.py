...
def _apply_tool_runtime_override(config: Any, *, sandbox_tools: bool) -> str:
    if sandbox_tools:
        ...
        os.environ["AGENT_REMOTE_SANDBOX"] = "1"
    ...
    while True:
        try:
            ...
            if event.event_type == "ready":
                ...
                print_turn_complete()
                session = session_holder[0] if session_holder else None
                print_plan(getattr(session, "current_plan", None))
...
async def main(model: str | None = None, sandbox_tools: bool = False):
    ...
    # HF token — required for Hub tools; optional for custom-inference-only LLMs.
...
async def headless_main(
    ...
    if not hf_token:
        if model_uses_custom_inference_endpoint(config.model_name):
            print(
                "No HF_TOKEN: using custom LLM only; Hub tools may fail.",
                file=sys.stderr,
            )
        elif not is_local_model_id(config.model_name) or not local_mode:
            print(
                "ERROR: No HF token found. Set HF_TOKEN, run `huggingface-cli login`, "
                "or configure CUSTOM_INFERENCE_* for a matching model.",
                file=sys.stderr,
            )
            sys.exit(1)
    elif hf_token:
