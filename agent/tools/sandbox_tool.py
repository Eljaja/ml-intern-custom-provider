...
Call sandbox_create before using operation tools when no sandbox is active.
"""
...
SANDBOX_CREATE_TOOL_SPEC = {
    ...
    "description": (
        ...
        "Workflow: sandbox_create → write script → pip install → test with small run → fix errors → promote when satisfied.\n"
        "The sandbox persists across tool calls within the session. pip install works out of the box.\n\n"
        "Use this when: you need to develop, test, and iterate on scripts in a remote sandbox. "
